<template>
      ...
      <div class="hero-downloads animate-in delay-3">
        <a :href="downloadUrls?.windows || '#'" class="btn btn-download" target="_blank" rel="noopener noreferrer">
          ...
          <div class="btn-download-inner">
            <span>Windows</span>
            <small class="btn-download-note">&nbsp;</small>
          </div>
        </a>
        <a :href="downloadUrls?.mac || '#'" class="btn btn-download" target="_blank" rel="noopener noreferrer">
          ...
          <div class="btn-download-inner">
            <span>macOS <span class="beta-badge">BETA</span></span>
            <small class="btn-download-note">{{ t('download.beta_note') }}</small>
          </div>
        </a>
        <a :href="downloadUrls?.linux || '#'" class="btn btn-download" target="_blank" rel="noopener noreferrer">
          ...
          <div class="btn-download-inner">
            <span>Linux <span class="beta-badge">BETA</span></span>
            <small class="btn-download-note">{{ t('download.beta_note') }}</small>
          </div>
        </a>
      </div>
  ...
  <!-- ====== CHECKOUT BETA CONFIRMATION MODAL ====== -->
  <div v-if="showCheckoutModal" class="checkout-modal-overlay" @click.self="cancelCheckoutModal()">
    <div class="checkout-modal-box">
      <div class="checkout-modal-icon">⚠️</div>
      <h3>{{ t('checkout.beta_modal.title') }}</h3>
      <p>{{ t('checkout.beta_modal.body') }}</p>
      <div class="checkout-modal-actions">
        <button class="btn btn-ghost" @click="cancelCheckoutModal()">{{ t('checkout.beta_modal.cancel') }}</button>
        <button class="btn btn-primary" @click="confirmCheckout()">{{ t('checkout.beta_modal.confirm') }}</button>
      </div>
    </div>
  </div>
</template>
